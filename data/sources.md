...
## The registry was reconciled against what verification found — 2026-08-16

`data/sources.registry.json` was written before any sponsor page had been read, and by this
point most of its assumptions were wrong in ways that would have misdirected the next pass.
Reconciled at `schema_version: 1.1`. What changed, and why each mattered:

| Stale claim | What verification found |
| --- | --- |
| NCJ runs 5 Sprints incl. SSB | **CW and RTTY only.** No SSB Sprint exists as of 2026. |
| NCCC Sprint is NCJ's | It is **NCCC's** — different sponsor, different rules page. |
| 10-10 runs 4 QSO Parties | **Three.** The fourth was never a 10-10 event. |
| AGCW "Goldene Taste" is a contest | It is an **award**, not a contest. Nothing to encode. |
| FISTS sprints are live | **Suspended from 2026.** Records keep `active_until: 2025` so past years still answer, and are counted as retired, not as coverage. |
| QRP ARCI pending | **Blocked at source** — see below. More effort will not fix it. |
| SARL a transient 503 | **Dead host** — expired TLS certificate, a durable blocker. |
| Tier 4 and CQ in progress | **Complete.** |

Two errors this surfaced that nothing else would have:

- **21 ARRL records carried no `country` at all.** Any regional rollup would have silently
  under-counted North America by a quarter of the catalog. Filled, and `ZS`/`EI` (amateur
  prefixes) normalised to `ZA`/`IE` (ISO 3166-1) so one vocabulary is in use.
- **IRTS and K1USN Radio Club were in the catalog with no registry entry whatsoever.** The
  registry now declares each org's `catalog_sponsors`, and a test in both engines fails on a
  sponsor with no entry — so that class of drift is mechanically detectable rather than
  needing someone to notice.

The structural fix is that **counts are no longer hand-maintained.** `estimated_total` stays,
explicitly labelled a pre-research guess that is neither a target nor a denominator;
everything describing what the catalog actually contains is generated by
`scripts/coverage.py` and re-derived independently by `test_registry_coverage_is_current` in
both engines. The generator does not verify its own output.

What the first run showed: **60 of 84 records are North American (71%)**, 19 European, 4
international, 1 African, and **Asia, Oceania and South America are empty**. Tiers 2 and 3
have one worked org each out of 20 and 10. Tier 5 is 50 more US QSO parties, so working it
