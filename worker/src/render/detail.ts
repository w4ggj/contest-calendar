...
function takeSection(
  ...
  // calendar now, then the feed, then the raw record.
  return (
    ...
    (gcal
      ? `<p class="note"><strong>Add to Google Calendar</strong> puts the next ` +
        `running in as a single event. It is one event, not a subscription — ` +
        `come back for the one after it, or use the feed below.</p>`
      : "") +
    ...
    `does not duplicate anything: <code class="feed">${esc(feed)}</code></p>` +
    // Google has no working one-click subscribe -- its documented cid deep link
    // was tried and refused. The manual box is the only route that works, and a
    // reader who does not know that will assume the feed is broken.
    `<p class="note"><strong>Google has no working one-click subscribe.</strong> ` +
    `Paste that address into Settings → Add calendar → <strong>From URL</strong>. ` +
    `Google then polls it on its own schedule, <strong>often 8–24 hours, and ` +
    `that cannot be forced</strong> — so use Add to Google Calendar above if you ` +
    `want the next running in your calendar now.</p>` +
    `</section>`
  );
}
