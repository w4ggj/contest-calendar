...
def test_unrecorded_bands_are_the_documented_exception(catalog):
    ...
    darc-hell-kurz and darc-cw-ausbildungscontest joined on 2026-08-23 for a
    THIRD reason, which is worth distinguishing from the other two. Neither
    sponsor said "all bands" and neither source is unreachable: DARC simply
    publishes no Ausschreibung for the HELL-Kurzcontest at all, and the CW
    training contest's page is prose about teaching CW rather than rules. The
    dates and times come from DARC's own Termine table, which carries no bands.
    Both records are `verified: false` and say so. If DARC ever publishes rules
    pages for them, they leave this list.
    """
    unrecorded = sorted(c["id"] for c in catalog if not c.get("bands"))
    assert unrecorded == ["darc-cw-ausbildungscontest",
        "darc-hell-kurz",
